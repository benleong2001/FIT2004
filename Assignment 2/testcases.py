import unittest
from assignment2_chk import RoadGraph, optimalRoute  # change appropriately
# from Snowboarding import optimalRoute
...
# The tests given on the assessment 2 sheet
class TestRoadmapBasic(unittest.TestCase):
...
# testing scenarios where None should be returned
class TestRoadmapIsNone(unittest.TestCase):
...
# testing the disconnected graph
class TestRoadmapDisconnected(unittest.TestCase):

    def setUp(self) -> None:
        self.disconnected_graph = RoadGraph(
            roads_disconnected, cafes_disconnected)
...
class TestRoadmapCircular(unittest.TestCase):  # testing the disconnected graph
    ...
    def test_circular_2(self):
        self.assertEqual([0, 1, 2, 3, 4, 0, 1, 2, 3],
                         self.circular_graph.routing(0, 3))


# testing the unreachable cafe graph
class TestRoadmapUnreachable(unittest.TestCase):

    def setUp(self) -> None:
        self.unreachable_graph = RoadGraph(
            roads_unreachable, cafes_unreachable)
